<?php
// Parcial de campos (v1.5.2) — Ajustes: quitar paga_coseguro, quitar Detalle de coseguro (si aplica),
// quitar Descripción general y dejar Código en un solo renglón.

// Helpers locales por si no están definidos en este scope
$sel = $sel ?? function($val,$cur){ return ($val===$cur)?'selected':''; };
$getv = $getv ?? function($key, $default='') use ($mutual, $data){
  if (isset($mutual[$key])) return $mutual[$key];
  if (isset($data[$key])) return $data[$key];
  return $default;
};

// Opciones comunes (conservamos las de v1.5.2)
...
<div class="row g-3 mt-3">
  ...
  <div class="col-md-6">
    ...
    <input type="number" min="0" class="form-control" name="validez" value="<?= htmlspecialchars($getv('validez','30')) ?>">
  </div>

  <!-- OJO: quitamos "Detalle de coseguro (si aplica)" -->
  ...
  <!-- OJO: quitamos "Descripción general" (campo viejo) -->
  ...
  <div class="col-md-6">
    ...
    <?php if (!empty($mutual['adj_listado_estudios'])): ?>
      <div class="mb-1">
        <a target="_blank" href="<?= $appUrl ?>/uploads/<?= htmlspecialchars($mutual['adj_listado_estudios']) ?>">Ver archivo actual</a>
      </div>
      <input type="hidden" name="adj_listado_estudios_existing" value="<?= htmlspecialchars($mutual['adj_listado_estudios']) ?>">
    <?php endif; ?>
  ...
  <div class="col-md-6">
    ...
    <?php if (!empty($mutual['adj_normas_operativas'])): ?>
      <div class="mb-1">
        <a target="_blank" href="<?= $appUrl ?>/uploads/<?= htmlspecialchars($mutual['adj_normas_operativas']) ?>">Ver archivo actual</a>
      </div>
      <input type="hidden" name="adj_normas_operativas_existing" value="<?= htmlspecialchars($mutual['adj_normas_operativas']) ?>">
    <?php endif; ?>
